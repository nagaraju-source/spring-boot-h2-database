...
import com.example.microservices.entity.Tutorial;
import com.example.microservices.repository.TutorialRepository;
...
public class TutorialControllerTests {
	...
	void shouldReturnNotFoundTutorial() throws Exception{
		...
		mockMvc.perform(get("/api/tutorials/{id}",id)).andExpect(status().isNotFound())
		.andDo(print());
	...
	void shouldReturnListOfTutorials() throws Exception{
		List<Tutorial> tutorials = new ArrayList<>(Arrays.asList(
				new Tutorial(1, "Spring Boot @WebMvcTest 1", "Description 1", true),
	...
	void shouldReturnlistOfTutorialsWithFilter() throws Exception
	{
		List<Tutorial> tutorials = new ArrayList<>(
		    ...
		    mockMvc.perform(get("/api/tutorials").params(paramMap))
		    .andExpect(status().isOk())
		    .andExpect(jsonPath("$.size()").value(tutorials.size()))
		    .andDo(print());
	...
	void shouldDeleteAll() throws Exception{
		//doNothing().when(tutorialRepository);
